...
from deep_translator import GoogleTranslator  # Import at the top
...
class Application:
    ...
    def __init__(self):
        ...
        # Select multiple voices (change indexes as per your system)
        self.selected_voices = [
            self.voices[0].id,  # English (US)
      
        ]
        
        self.current_voice_index = 0  # Start with the first voice
        self.speak_engine.setProperty("voice", self.selected_voices[self.current_voice_index])
        ...
        self.root = tk.Tk()
        self.root.title("AI-Driven Gesture Recognition With Multilingual Translation")
        ...
        self.T.config(text="AI-Driven Gesture Recognition ", font=("Courier", 30, "bold"))
        ...
        self.change_voice = tk.Button(self.root)
        self.change_voice.place(x=1205, y=30)  # Adjust position as needed
        self.change_voice.config(text="Change Voice", font=("Courier", 20), command=self.switch_voice)

        self.speak_english = tk.Button(self.root, text="Speak English", font=("Courier", 20),
                               command=lambda: self.speak_fun('en'))
        self.speak_english.place(x=1305, y=560)

        self.speak_hindi = tk.Button(self.root, text="Speak Hindi", font=("Courier", 20),
                             command=lambda: self.speak_fun('hi'))
        self.speak_hindi.place(x=1305, y=660)

        self.speak_kannada = tk.Button(self.root, text="Speak Kannada", font=("Courier", 20),
                               command=lambda: self.speak_fun('kn'))
        self.speak_kannada.place(x=1305, y=460)
    ...
    def switch_voice(self):
        self.current_voice_index = (self.current_voice_index + 1) % len(self.selected_voices)
        self.speak_engine.setProperty("voice", self.selected_voices[self.current_voice_index])

        print("Switched to:", self.selected_voices[self.current_voice_index])
        self.speak_engine.say("Voice changed successfully.")
    ...
    def speak_fun(self, language='en'):
     text = self.str.strip()
     if not text:
        print("No text to speak")
        return

     if language == 'en':  # Speak in English using pyttsx3
        self.speak_engine.say(text)
        self.speak_engine.runAndWait()
     else:  # Translate and speak using gTTS
        translated_text = self.translate_text(language)  # Get translated text
        print(f"Translated Text ({language}):", translated_text)
        tts = gTTS(text=translated_text, lang=language)
        tts.save("translated_audio.mp3")
        os.system("start translated_audio.mp3")  # Windows: 'start', Linux: 'mpg321'


    def translate_text(self, target_lang='hi'):  # Default: Hindi
     if not self.str.strip():
        print("No text to translate")
        return ""

     translated_text = GoogleTranslator(source='auto', target=target_lang).translate(self.str)
